...
public struct BehaviourResponse: Codable {
    ...
    public init(from behaviour: Behaviour) {
        self.init(id: behaviour.id, name: behaviour.name, min: behaviour.min, max: behaviour.max, numberOfDetails: behaviour.numberOfDecimals, orientation: behaviour.orientation, options: behaviour.options, createdAt: behaviour.createdAt, updatedAt: behaviour.updatedAt, deletedAt: behaviour.deletedAt)
    }
}
